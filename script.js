function mostrarRespuesta(opcion) {
  ...
  if (opcion === 1) {
    respuesta.textContent = "¡Correcto! Llamar al 911 es la mejor opción.";
...
const quizData = [
  {
    question:
      "¿Qué deberías hacer si ves a un niño solo y asustado en la calle?",
    choices: [
  ...
  {
    question:
      "¿Qué información es más útil para las autoridades si presencias un secuestro?",
    choices: [
  ...
  {
    question:
      "¿Cuál es la mejor forma de actuar si un desconocido te ofrece llevarte a casa?",
    choices: [
  ...
  {
    question:
      "¿Qué deberías hacer si recibes un mensaje sospechoso en redes sociales?",
    choices: [
  ...
  {
    question:
      "¿Qué es lo primero que debes hacer si presencias un intento de secuestro?",
    choices: [
      "Grabar la situación con tu celular.",
...
function selectAnswer(selectedIndex, correctIndex) {
  ...
  if (selectedIndex === correctIndex) {
    ...
    const q = quizData[currentQuestion];
    resultEl.textContent = `Incorrecto. La respuesta correcta era: "${
      q.choices[q.correct]
    }"`;
...
nextBtn.onclick = () => {
  ...
  if (currentQuestion < quizData.length) {
    ...
    choicesEl.innerHTML = "";
    nextBtn.style.display = "none";
    resultEl.textContent = "Gracias por aprender y ayudar a concientizar.";
...
// Scrollspy para índice de navegación
document.addEventListener("scroll", () => {
  ...
  document.querySelectorAll("#indice-navegacion a").forEach((a) => {
